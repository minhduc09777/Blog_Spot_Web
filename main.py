...
def register():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            result = db.session.execute(db.select(User).filter_by(mail=form.email.data)).scalars()
            if result.all():
                flash("You have already signed up with that email, please log in instead.")
            else:
                new_user = User(
                    mail=form.email.data,
                    password= generate_password_hash(form.password.data, method="pbkdf2:sha512", salt_length=8),
                    name=form.name.data
                )
...
def login():
    form = LoginForm()
    if request.method == "POST":
        result = db.session.execute(db.select(User).filter_by(mail=form.email.data))
        user = result.scalar()
        if user:
